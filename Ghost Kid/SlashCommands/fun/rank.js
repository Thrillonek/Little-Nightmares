...
module.exports = {
  name: 'rank',
  description: 'Ukáže vám váš rank, XP a level.',
  options: [
    {
      name: "člen",
      description: "Člen, od kterého rank bude (pokud nechcete vědět svůj).",
      type: "USER",
      required: false,
    }
  ],
  async execute(int, args, client, discord){

    let u = args.get("člen") || int.member;
    let avatar = int.user.displayAvatarURL({ dynamic: true });

    let result = await economy.findOne({ userID: u.user.id })
    ...
    if(!result){
      ...
      for(let i = 0; i < lb.length; i++){
        if(lb[i].userID === u.user.id) uRank = i+1;
      }
      ...
      ctx.fillStyle = "#000";
      ctx.font = "35px Arial";
      ctx.fillText(`${u.user.username}     #${u.user.discriminator}`, 215, 95);
      ctx.closePath();
      ...
      ctx.closePath();
      ctx.clip();
      ctx.drawImage(await Canvas.loadImage(u.user.displayAvatarURL({ format: "png" })), 50, 50, 150, 150)

      const attachment = new discord.MessageAttachment(canvas.toBuffer(), 'Rank.png');

      int.followUp({ files: [attachment] });
    }
  }
}